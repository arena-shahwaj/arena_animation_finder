...
function searchByInterest() {
    ...
    const filteredCourses = courses.filter((course) =>
        course.name.toLowerCase().includes(interestInput) // Use includes for better matching
    );
    displayCourses(filteredCourses);

    const resultText = document.getElementById("result-text"); // Add result display
    if (filteredCourses.length > 0) {
        resultText.textContent = `Results Found: ${filteredCourses.length}`;
